...
- 上传过程带进度条反馈
- 阅读页会显示元数据识别状态，并在元数据补全后自动刷新基础信息
- 为论文创建阅读工作区，生成：
  - `paper.json`
  - `analysis.json`
  - `qa_history.json`
...
7. 创建阅读工作区
8. 阅读页顶部会显示元数据识别状态，补全完成后自动刷新标题、作者、venue、年份和 DOI
9. 在阅读页手动触发深度分析，生成中文结构化 `analysis.json`
10. 分析结束后，把 `research_theme` 合并进 citation `tags`
11. `/keywords` 页面会同时展示原始搜索关键词和深度阅读回写关键词
12. 如果从深度阅读库删除该文献，会清除该阅读条目的全部相关派生数据；共享 PDF 会保留给其他文献继续使用
